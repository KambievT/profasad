...
export const CatalogProductCard = memo(({ product, extractPrice }: CatalogProductCardProps) => {
  ...
  const cleanName = (name: string) => {
    return String(name || "").replace(/\s*-\s*купить с доставкой по Екатеринбургу$/i, "").trim()
  }
  ...
  return (
    <Dialog>
      ...
      <DialogContent className="sm:max-w-2xl p-4 sm:p-6">
        <div className="grid gap-4 md:gap-6 md:grid-cols-2 items-center">
          ...
          <div className="flex flex-col gap-2 sm:gap-3">
            <DialogHeader className="p-0">
              ...
              <DialogTitle className="text-base sm:text-lg md:text-xl leading-snug">{cleanName(product.name)}</DialogTitle>
            </DialogHeader>
          </div>
        </div>

        <div className="mt-2 flex flex-col sm:flex-row sm:items-center justify-between gap-3">
          <div className="text-lg sm:text-2xl font-bold text-primary">{formatPrice(extractPrice)}</div>
          <div className="flex flex-col sm:flex-row items-stretch sm:items-center gap-2 w-full sm:w-auto">
            <Button asChild className="rounded-full w-full sm:w-auto">
              <a href="https://wa.me/79000000000" target="_blank" rel="noreferrer">Написать в Whatsapp</a>
            </Button>
            <DialogClose asChild>
              <Button variant="secondary" className="w-full sm:w-auto">Закрыть</Button>
            </DialogClose>
          </div>
        </div>
      </DialogContent>
